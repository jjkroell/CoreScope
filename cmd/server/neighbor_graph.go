...
// number of distinct observers that have contributed to this edge. Edges
// corroborated by multiple independent observers outrank edges seen by a single
// observer at the same raw score. Issue #1229 (Option C).
...
type NeighborGraph struct {
	...
	// endpoints had GPS and their haversine distance exceeded the configured
	// threshold. Accessed via sync/atomic. See issue #1228.
	RejectedEdgesGeoFar uint64

	// maxEdgeKm is the geo-sanity threshold copied from config at build time.
	// 0 means "no limit" / filter disabled.
	maxEdgeKm float64

	// nodeGeo maps lowercased pubkey → geo info for upsertEdge sanity checks.
	// Populated by the builder; empty for graphs constructed via NewNeighborGraph.
...
// DefaultMaxEdgeKm is the conservative built-in cap for the geo-implausibility
// filter (issue #1228). 500 km is well above any plausible terrestrial LoRa hop.
...
func (g *NeighborGraph) upsertEdge(pubkeyA, pubkeyB, prefix, observer string, snr *float64, ts time.Time) {
	...
	// the haversine distance exceeds the configured threshold, drop the edge.
